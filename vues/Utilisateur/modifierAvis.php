<?php
$titre = "UrbanStyle: Utilisateur - Modifier un avis";
ob_start();

/**
 * @var array $avis
 */ // Enlève l'erreur visuel de VSCode sur les variables
?>

<h1 class="text-center text-decoration-underline mt-3 mb-4">Modifier mon avis</h1>

<form action="index.php?action=Avis_update" method="post">

    <input type="hidden" name="id_avis" value="<?= $avis['id_avis'] ?>">
    <input type="hidden" name="id_produit" value="<?= $avis['id_produit'] ?>">
    <input type="hidden" name="retour" value="<?= $_GET['retour'] ?? '' ?>">

    <div class="mb-3">
        <label class="form-label">Note</label>
        <select name="note" class="form-select">

            <?php for ($i = 1; $i <= 5; $i++) { ?>
                <option value="<?= $i ?>" <?= $avis['note'] == $i ? 'selected' : '' ?>>
                    <?= $i ?> ⭐
                </option>
            <?php } ?>

        </select>
    </div>

    <div class="mb-3">
        <label class="form-label">Commentaire</label>
        <textarea name="commentaire" class="form-control"><?= htmlspecialchars($avis['commentaire']) ?></textarea>
    </div>

    <button type="submit" class="btn btn-success">
        Modifier l'avis
    </button>

    <a href="index.php?action=detail_produit&id=<?= $avis['id_produit'] ?>"
       class="btn btn-secondary">
        Annuler
    </a>

</form>

<?php
$contenu = ob_get_clean();
require "vues/template.php";
?>